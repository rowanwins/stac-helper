...
export function createStacItemFromDataAndType (data, stacType, parent) {
    if (stacType === 'Catalog') {
        return new Catalog(data, parent)
    } else if (stacType === 'Collection') {
        return new Collection(data, parent)
    } else if (stacType === 'Item') {
        return new Item(data, parent)
    }
    return null
}